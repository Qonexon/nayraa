...
SYSTEM_PROMPT_FINDINGS = (
    ...
    "\n"
    "Check specifically for language-agnostic structural, architectural, and state "
    "defects:\n"
    "- Granularity and blast-radius mismatches: filtering or mutating an individual "
    "entity in a\n"
    "  way that unintentionally mutates or excludes sibling entities or parent "
    "containers.\n"
    "- Polarity and fail-open inversions: replacing an explicit allowlist with a "
    "subtractive\n"
    "  blocklist or tombstone state that silently leaks unvetted future additions.\n"
    "- Precedence and shadowing collisions: higher-scope or default rules that "
    "unconditionally\n"
    "  suppress lower-scope overrides or make them unreachable.\n"
    "- Incomplete state-space expansions: adding new enum, union, or status variants "
    "where\n"
    "  existing matchers or callers fall through to unintended fallback behavior.\n"
    "- Edge-only invariant enforcement: enforcing business rules only in presentation "
    "or list\n"
    "  queries while mutation, direct-fetch, or background pipelines bypass them.\n"
    "\n"
...
SYSTEM_PROMPT_VERDICT = (
    ...
    "a\n"
    "caller-side check?\n"
    "4. For architectural or blast-radius claims, is there an existing scope "
    "constraint or\n"
    "filter that prevents the claimed sibling exclusion, leak, or state collision?\n"
    "5. Does the finding depend on code that is not in the context? "
    "If so it is speculation.\n"
    "6. Would this fire on the code as it existed before this diff? "
