...
def _diff() -> str:
    return "diff --git a/api/export.py b/api/export.py"
...
def test_unjustified_objection_survives():
    ...
    result = passes.review_shape(client, _diff(), SHAPE)
    assert len(result) == 1
...
def test_justified_objection_dropped():
    ...
    assert passes.review_shape(client, _diff(), SHAPE) == []
...
def test_shape_reaches_the_model():
    ...
    passes.review_shape(client, _diff(), SHAPE)
...
def test_evidence_that_is_not_a_changed_path_is_dropped():
    ...
    result = passes.review_shape(client, _diff(), GROUNDED_SHAPE)
    assert result == []
    assert len(client.calls) == 1
